...
- 仓库在 `test/support` 提供无测试框架依赖的契约探针，仓库内所有持久化实现必须运行；探针不属于生产 API。
...
删除未发布的 `sessions/testing` 公共入口，契约探针迁入 `test/support`。协议继续完整描述第三方 Store 必须满足
的原子性、分页和错误语义，但外部项目应在自己的测试目录实现这些断言。若以后发布统一扩展开发工具包，需把
模型和 Store 等契约测试作为一个整体设计，而不是让测试代码散落在生产子模块中。
