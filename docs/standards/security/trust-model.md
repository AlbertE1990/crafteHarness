...
| 未审查的第三方工具代码         | 不可信       | 当前不应加载到 CraftAgent 所在的 Node.js 进程 |

这里的关键边界是：**当前 Tool Harness 不是 JavaScript 沙箱。**

如果应用开发者已经可以修改并运行 Node.js 代码，他也可以绕开 CraftAgent 直接调用 `rm`、`fs.rm()`
...
```mermaid
flowchart LR
  Agent[CraftAgent] --> Broker[受控 Tool Broker]
